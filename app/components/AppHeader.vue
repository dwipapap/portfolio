...
<template>
  <!-- Desktop Header: Original floating pill design -->
  ...
  <header
    class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-gray-900/95 border-t border-gray-200 dark:border-gray-800 pb-[env(safe-area-inset-bottom)]"
  >
    <nav class="flex justify-around items-end h-16 w-full px-2">
      <button
        v-for="item in navItems"
        :key="item.label"
      ...
      >
        <ClientOnly>
          <UIcon
            ...
            class="size-6 transition-transform duration-200 group-active:scale-95"
          />
          <span class="text-[10px] font-medium">{{ isDark ? 'Dark' : 'Light' }}</span>
          <div class="w-1 h-1 mt-0.5 bg-transparent" />

          <template #fallback>
            <div class="size-6 bg-gray-200 dark:bg-gray-800 rounded-full animate-pulse" />
          </template>
        </ClientOnly>
      </button>
    </nav>
